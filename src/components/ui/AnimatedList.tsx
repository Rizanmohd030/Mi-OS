    ...
    <div className={`relative ${className}`}>
      <div
        ...
        className={`max-h-[400px] overflow-y-auto p-3 ${
          displayScrollbar
            ? '[&::-webkit-scrollbar]:w-[6px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-[#BDDDFC] [&::-webkit-scrollbar-thumb]:rounded-[3px]'
            : 'scrollbar-hide'
        }`}
        onScroll={handleScroll}
        style={{
          scrollbarWidth: displayScrollbar ? 'thin' : 'none',
          scrollbarColor: '#BDDDFC transparent'
        }}
      >
          ...
          >
            <div 
              className={`
                ...
                ${selectedIndex === index 
                  ? 'bg-[#BDDDFC] border border-[#88BDF2]' 
                  : 'bg-blue-50 border border-[#BDDDFC] hover:bg-white'
                }
                ${itemClassName}
              `}
            >
              <input
                ...
                readOnly
                className="w-4 h-4 rounded border-[#BDDDFC] text-[#88BDF2] cursor-pointer"
              />
              <p className="text-sm font-light text-gray-900">{item}</p>
            </div>
          </AnimatedItem>
        ))}
      </div>
